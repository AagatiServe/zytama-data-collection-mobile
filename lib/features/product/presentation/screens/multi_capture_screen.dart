...
class _MultiCaptureScreenState extends State<MultiCaptureScreen> {
  static const _steps = [
    _StepConfig(
      label: AppStrings.productPhotoPlain,
      instruction: AppStrings.productPhotoInstruction,
      icon: Icons.inventory_2_rounded,
      color: Color(0xff0A6475),
    ),
    _StepConfig(
      label: AppStrings.ingredientsPhotoPlain,
      instruction: AppStrings.ingredientsPhotoInstruction,
      icon: Icons.list_alt_rounded,
      color: Color(0xff7B2FBE),
    ),
    _StepConfig(
      label: AppStrings.nutritionPhotoPlain,
      instruction: AppStrings.nutritionPhotoInstruction,
  ...
  Future<void> _capture() async {
    ...
    if (_step < _steps.length - 1) {
      ...
      // Dispose the controller before popping so the camera hardware is
      // fully released before the next screen (scanner or capture) opens it.
      final images = List<File>.from(_captured);
      final ctrl = _ctrl;
      _ctrl = null;
      if (mounted) setState(() {});
      await ctrl?.dispose();
            ...
            child: Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    AppStrings.stepOf(stepNumber, totalSteps, step.label),
                    style: TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
                      color: step.color,
...
class _BottomSheetClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.lineTo(start, 0);
    path.cubicTo(
        start + 10, 0, start + 15, notchDepth, size.width / 2, notchDepth);
